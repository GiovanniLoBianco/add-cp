...
public class ADDPropagatorXP {
	...
	public static ArrayList<Integer>[] addIntegerVariablesDD(Model model, int[][] domains,
		...
		for (int i = 0; i < n; i++) {
			...
			if (areDomainsConstrained) {
				// Create ADD representing x_i as its boolean sum
				// decompositions
				ADD domain_i = new ADD(idxBooleanVarLists[i]);
				...
				// Create ADD constraint, binding variables of the model to
				// ADD structure
				...
				Constraint cstr_domain_i = new Constraint("Domain_" + i,
						new ADDPropagator(booleanVars_i, domain_i, bounded_x_i, mapVarIndex_Domains[i]));
				cstr_domain_i.post();
			}
		}
	...
	public static Model[] generateRandomLinearModel(int n, int nc, int m, int l_dom, int u_dom, int l_coeff,
		...
		for (int j = 0; j < nc; j++) {
			...
			Constraint arithm = new Constraint("c_" + j, new ADDPropagator(usedBoolVars, dd, ineq, mapBoolVarIndex));
			arithm.post();
	...
	public static Model[] generateRandomQuadraticModel(int n, int nc, int m, int l_dom, int u_dom, int l_coeff,
		...
		for (int j = 0; j < nc; j++) {
			...
			IntVar[] usedBoolVars = new IntVar[boolVarSet.size()];
			usedBoolVars = boolVarSet.toArray(usedBoolVars);
			Constraint quad = new Constraint("c_" + j, new ADDPropagator(usedBoolVars, dd, ineq, mapBoolVarIndex));
			quad.post();
